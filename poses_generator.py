...
def gen_rader_rays(image,h_r,theta,pixel,Na,Nr,perturb,N_theta = 20,scale=1):
    ...
    I = V_r.reshape(1, 1, 1, 3, Na)
    I = I.transpose(4, 1, 2, 3, 0)
    I = np.repeat(I, Nr, axis=1)
    I = np.repeat(I, N_theta, axis=2)


    J = np.array([i for i in range(Nr)])
    J = ( J + 1 - (Nr+1)/2 ) * [delta_r] + Rc

    J = J.reshape(1, 1, 1, 1, Nr)
    J = J.transpose(0, 4, 2, 3, 1)
    J = np.repeat(J, Na, axis=0)
    J = np.repeat(J, N_theta, axis=2)


    K = D_r.reshape(1, 1, 1, 3, N_theta)
    K = K.transpose(0, 1, 4, 3, 2)
    K = np.repeat(K, Na, axis=0)
    K = np.repeat(K, Nr, axis=1)

    temp = I + np.matmul(K, J)

    Rr = Rr.reshape(1, 1, 1, 3, 3)
    Pr = Pr.reshape(1, 1, 1, 3, 1)
    temp2 = np.matmul(Rr, temp) + Pr
    V_w_sample_new = temp2.transpose(4, 3, 0, 1, 2)
    V_w_sample = V_w_sample_new[0, :, :, :, :]

    # print(V_w_sample==V_w_sample_new2)
    # print((V_w_sample==V_w_sample_new2).mean())
    # print(V_w_sample.shape, V_w_sample_new2.shape)
